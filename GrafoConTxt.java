...
public class GrafoConTxt {
	...
	/*
	 * Este método recibe un grafo no dirigido y la matriz de selecciones (matriz
	 * de adyacencia) para asociar los vertices del grafo (crearArista)
	 */
	...
	// Metodo que añade las aristas en el grafo (lista de adyacencia) dado los
	// vertices adyacentes
	void crearArista(int v, int w) {
		// Grafo no dirigido, por eso se unen con aristas en ambos sentidos entre par de
		// vertices
		ady[v].add(w);
		ady[w].add(v);
	}
	...
	/*
	 * Asigna las villas indexadas desde 0 a todos los vertice e imprime una matriz
	 * que contiene las villas asignadas a cada pais, donde n son las filas (villas
	 * a construir) y m las columnas (selecciones).
	 */
	void construirVilla() {
		int[][] S = new int[V][V]; /*
									 * * Matriz solucion: S[i,j]= 1 si en la villa i se aloja la selección j, en
									 * otro caso S[i,j] = 0.
									 */
		int resultado[] = new int[V]; // Arreglo utilizado para el numero de villas

		/*
		 * Se inicializa todos los vertices sin signo para indicar que aun ninguna
		 * seleccion tiene asociada una villa para alojarse
		 */
		Arrays.fill(resultado, -1);

		// Se asigna directamente la primer villa al primer pais ya que no existe
		// problema
		resultado[0] = 0;

		/*
		 * Arreglo Booleano utilizado para almacenar las villas disponibles. false:
		 * villa ya asignada a un vertice adyacente (seleccion) true: villa disponible
		 * para asignar a un vertice adyacente (seleccion)
		 */
		boolean villaDisponible[] = new boolean[V];
		Arrays.fill(villaDisponible, true);

		// Construye o asigna villas a las selecciones restantes que aun no tienen
		// ninguna villa asignada
		for (int u = 1; u < V; u++) { // itera sobre todos los vertices adyacentes
			...
			while (it.hasNext()) {
				...
				if (resultado[i] != -1) {
					...
					/*
					 * Si una determinada seleccion ya tiene una villa asignada la pone como no
					 * disponible (false) para sus selecciones adyacentes (selecciones que no tienen
					 * relaciones diplomaticas) para no asingarle la misma villa
					 */
				}
			}
